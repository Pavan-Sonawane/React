...
namespace MyAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
       ...
       [Route("UpdateOrder/{orderId}")]
        [HttpPut]
        public async Task<IActionResult> UpdateCategory(int orderId, OrderUpdateModel categoryModel)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        // }
        [HttpGet("{orderId}/products")]
public async Task<ActionResult<IEnumerable<Product>>> GetProductsByOrderId(int orderId)
{
    try
    {
        var order = await _serviceCategory.Get(orderId);
